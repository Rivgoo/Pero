...
namespace Pero.Languages.Uk_UA.Dictionaries.Parsing;

public static class UkTagParser
{
	private const char TagSeparator = ':';

	public static MorphologyTagset Parse(string tagString)
	{
		var parts = tagString.Split(TagSeparator);
		if (parts.Length == 0) return new MorphologyTagset(PartOfSpeech.Unknown);
		...
		return new MorphologyTagset(pos, caseType, gender, number, animacy, aspect, tense, person, features);
	}
	...
	private static bool TryParseNumber(string tag, out GrammarNumber result)
	{
		result = tag switch
		{
			"s" => GrammarNumber.Singular,
			"p" => GrammarNumber.Plural,
			_ => GrammarNumber.None
		};
	...
	private static bool TryParseAnimacy(string tag, out GrammarAnimacy result)
	{
		result = tag switch
		{
			"anim" => GrammarAnimacy.Animate,
			"inanim" => GrammarAnimacy.Inanimate,
			"unanim" => GrammarAnimacy.Unspecified,
			_ => GrammarAnimacy.None
		};
	...
	private static bool TryParseTense(string tag, out GrammarTense result)
	{
		result = tag switch
		{
			"past" => GrammarTense.Past,
			"pres" => GrammarTense.Present,
			"futr" => GrammarTense.Future,
			_ => GrammarTense.None
		};
	...
	private static bool TryParsePerson(string tag, out GrammarPerson result)
	{
		result = tag switch
		{
			"1" => GrammarPerson.First,
			"2" => GrammarPerson.Second,
			"3" => GrammarPerson.Third,
			_ => GrammarPerson.None
		};
