...
public sealed class UkrainianPenaltyMatrix : BasePenaltyMatrix
{
	private const float MaxKeyboardDistance = 10.5f;

	private static readonly char[] UkrainianAlphabet =
	{
	...
	public UkrainianPenaltyMatrix() : base(UkrainianAlphabet)
	{
	}

	protected override float CalculateSubstitutionCost(char expected, char actual)
	{
	...
	protected override int CharToIndex(char c)
	{
	...
	public override float GetInsertionCost(char c)
	{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public override float GetDeletionCost(char c)
	{
		if (IsApostrophe(c)) return 0.1f;
		if (c == 'ь' || c == 'й') return 0.4f;
		return 0.95f;
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public override float GetPositionalMultiplier(int currentIndex, int wordLength)
	{
