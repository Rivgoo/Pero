...
public class CompiledDictionary
{
	...
	public MorphologyTagset[] Tagsets { get; private set; } = Array.Empty<MorphologyTagset>();
	...
	public void Load(Stream inputStream)
	{
		...
		Tagsets = new MorphologyTagset[tagsetsCount];
		for (int i = 0; i < tagsetsCount; i++)
		{
			Tagsets[i] = new MorphologyTagset(
	...
	public IEnumerable<WordForm> GetAllForms(string lemma)
	{
		...
		foreach (var ruleId in _paradigms[paradigmId])
		{
			...
			yield return new WordForm(ApplyRule(lemma, rule), Tagsets[rule.TagId]);
		}
	}
