...
public readonly ref struct DamerauLevenshteinState
{
	...
	{
		currentBuffer[0] = _prevDistances[0] + context.PenaltyMatrix.GetDeletionCost(candidateChar);
		...
		for (int i = 1; i <= userLen; i++)
		{
			...
			float deleteCost = (candidateChar == _prevChar)
				...
				: _prevDistances[i] + (context.PenaltyMatrix.GetDeletionCost(candidateChar) * multiplier);

			float subCost = _prevDistances[i - 1] + (context.PenaltyMatrix.GetSubstitutionCostUnsafe(userChar, candidateChar) * multiplier);
