...
{
	private readonly IMorphologyTagParser tagParser;

	public DictionaryParser(IMorphologyTagParser tagParser)
	{
		this.tagParser = tagParser;
	}

	public DictionaryBuildData Parse(IEnumerable<string> rawLines, IReadOnlyDictionary<string, byte>? frequencies)
	{
		var data = new DictionaryBuildData();
		...
		foreach (var line in rawLines)
		{
			...
			var tagset = tagParser.Parse(tagString);
			if (!tagsetRegistry.TryGetValue(tagset, out var tagId))
			{
				tagId = (ushort)data.Tagsets.Count;
				tagsetRegistry[tagset] = tagId;
				data.Tagsets.Add(tagset);
			}
	...
	private void ProcessParadigms(DictionaryBuildData data)
	{
	...
	private void BuildTrees(DictionaryBuildData data, IReadOnlyDictionary<string, byte>? frequencies)
	{
